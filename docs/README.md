...
`archive/` содержит исторические документы, которые оставлены для справки,
но не являются инструкцией текущей рабочей схемы. В частности, там находятся
старое OSD-руководство и исследование платы контроллера.
