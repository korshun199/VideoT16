...
def запустить_без_окна(пересобрать: bool) -> int:
    """Создаёт видео без Tkinter, если графический дисплей недоступен."""
    if not пересобрать and ВИДЕО_БЕЗ_РАСПОЗНАВАНИЯ.is_file() and ВИДЕО_С_РАСПОЗНАВАНИЕМ.is_file():
        print(f"Чистое видео: {ВИДЕО_БЕЗ_РАСПОЗНАВАНИЯ}")
        print(f"Видео с распознаванием: {ВИДЕО_С_РАСПОЗНАВАНИЕМ}")
        return 0
    сообщения = queue.Queue()
    создать_два_видео(сообщения)
    ошибка = готово = None
    while not сообщения.empty():
        сообщение = сообщения.get()
        if сообщение[0] == "ошибка":
            ошибка = сообщение[1]
        elif сообщение[0] == "готово":
            готово = сообщение[1]
    if ошибка:
        ...
        return 1
    print(f"Готово: обработано фотографий — {готово}")
    print(f"Чистое видео: {ВИДЕО_БЕЗ_РАСПОЗНАВАНИЯ}")
    print(f"Видео с распознаванием: {ВИДЕО_С_РАСПОЗНАВАНИЕМ}")
    return 0
...
def main():
    """Запускает создание и сравнение видео с автоматическим headless-режимом."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--headless", action="store_true",
        help="создать видео без графического окна",
    )
    parser.add_argument(
        "--rebuild", action="store_true",
        help="заново прогнать все фотографии через модель",
    )
    args = parser.parse_args()

    # В SSH/контейнере Tkinter не имеет дисплея, но генерация видео должна
    # продолжать работать без ручного изменения исходника.
    headless = args.headless or not (os.environ.get("DISPLAY") or os.environ.get("WAYLAND_DISPLAY"))
    if headless:
        if not args.rebuild and ВИДЕО_БЕЗ_РАСПОЗНАВАНИЯ.is_file() and ВИДЕО_С_РАСПОЗНАВАНИЕМ.is_file():
            print(f"Чистое видео: {ВИДЕО_БЕЗ_РАСПОЗНАВАНИЯ}")
            print(f"Видео с распознаванием: {ВИДЕО_С_РАСПОЗНАВАНИЕМ}")
            return 0
        сообщения = queue.Queue()
    ...
    try:
        ...
        # DISPLAY может быть задан, но сам X-сервер недоступен.
        return запустить_без_окна(args.rebuild)
