...
class InavMspReader:
    ...
    def set_crosshairs_position(self, x: int, y: int) -> None:
        # Передаёт координаты штатного прицела Betaflight без изменения прошивки.
        from src.osd import set_crosshairs_position

        with self._write_lock:
            set_crosshairs_position(self._serial, x, y)
