...
class DetectionTracker:
    ...
    def update(self, detections: tuple[Detection, ...]) -> tuple[Detection, ...]:
        """Выбирает продолжение трека или кратко прогнозирует его положение."""
        if not detections:
            if self._last is None or self._missing >= self.max_missing:
                self._last = None
                return ()
            self._missing += 1
            ...
            self._last = replace(
                self._last,
                x1=round(self._last.x1 + dx),
                x2=round(self._last.x2 + dx),
                y1=round(self._last.y1 + dy),
                y2=round(self._last.y2 + dy),
                confidence=max(0.0, self._last.confidence - 0.02),
            )
            return (self._last,)
